...
# Source https://github.com/aws-samples/sagemaker-distributed-training-digital-pathology-images/blob/main/docker_utils.py
...
def build_and_push_docker_image(
    repository_name, dockerfile="Dockerfile", path=".", build_args={}, base_image=None
):
    ...
    _build_from_dockerfile(repository_name, dockerfile, path, build_args)
    ecr_tag = push(repository_name)
    return ecr_tag


def _build_from_dockerfile(
    repository_name, dockerfile="Dockerfile", path=",", build_args={}
):
    build_cmd = ["docker", "build", "-t", repository_name, "-f", dockerfile, path]
    for k, v in build_args.items():
        build_cmd += ["--build-arg", "%s=%s" % (k, v)]

    print(f"Building docker image {repository_name} from {dockerfile}")
    _execute(build_cmd)
    print(f"Done building docker image {repository_name}")
...
def _push(aws_account, aws_region, tag):
    ecr_repo = f"{aws_account}.dkr.ecr.{aws_region}.amazonaws.com"
    ecr_tag = f"{ecr_repo}/{tag}"
    _execute(["docker", "tag", tag, ecr_tag])
    print(f"Pushing docker image to ECR repository {ecr_repo}/{tag}\n")
    _execute(["docker", "push", ecr_tag])
    print(f"Done pushing {ecr_tag}")
    return ecr_tag
