...
def drop_low_confidence_preds(pred, gt, distance_tolerance=200, costly_dist=False):
    ...
    scenes = gt["scene_id"].unique()
    
    for scene_id in tqdm(scenes, total=len(scenes), desc='Dropping low confidence.'):
...
def compute_length_performance(preds, gt, tp_inds):
    ...
    for pair in tp_inds:
        ...
        pred_length = preds[pair["pred_idx"]]
        if isinstance(preds[pair["pred_idx"]], list) and len(preds[pair["pred_idx"]]) == 1:
            pred_length = preds[pair["pred_idx"]][0]
                    
        gt_pred = min(gt[pair["gt_idx"]], MAX_OBJECT_LENGTH_M)
        inf_pred = min(pred_length, MAX_OBJECT_LENGTH_M)
        pct_error += (
...
def score(pred, gt, shore_root, distance_tolerance=200, shore_tolerance=2, costly_dist=False):
    ...
    for scene_id in tqdm(scenes, total=len(scenes), desc='Computing F1-score for maritime object detection for each scene.'):
        pred_sc = pred[pred["scene_id"] == scene_id]
        gt_sc = gt[gt["scene_id"] == scene_id]
    ...
    _, _, loc_fscore = calculate_p_r_f(tp_inds, fp_inds, fn_inds)
    ...
    if shore_tolerance and shore_root:
        ...
        for scene_id in tqdm(scenes, total=len(scenes), desc='Computing close-to-shore F1 score for each scene.'):
            pred_sc = pred[pred["scene_id"] == scene_id]
...
def main(args):
    ...
    if args.scene_id is not None:
        inference = inference[inference["scene_id"] == args.scene_id].reset_index(drop=True)
        ground_truth = ground_truth[ground_truth["scene_id"] == args.scene_id].reset_index(drop=True)
    else:
        ground_truth = ground_truth[ground_truth["scene_id"].isin(inference["scene_id"].unique())].reset_index(drop=True)
    ...
    if not args.score_all:
        ...
        ground_truth = ground_truth[ground_truth["confidence"].isin(["HIGH", "MEDIUM"])].reset_index(drop=True)
